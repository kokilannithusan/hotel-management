...
const defaultCleaningTasks: Record<string, string[]> = activityDefinitionsDefaults.reduce((acc, def) => {
...
export const Housekeeping: React.FC<HousekeepingProps> = ({ mode }) => {
  ...
  const syncRoomsWithCleaningTasks = (tasksMap: Record<string, string[]>) => {
    setRooms((prevRooms) =>
      prevRooms.map((room) => {
        const existingLabels = new Set(room.activities.map((a) => a.label));
        const merged = [...room.activities];
        Object.keys(tasksMap).forEach((category) => {
          tasksMap[category].forEach((label) => {
            if (!existingLabels.has(label)) {
              // create a simple activity id from label
              const id = (label.toLowerCase().replace(/[^a-z0-9]+/g, "-") || `act-${Date.now()}`) as ActivityId;
              merged.push({ id, label, icon: Square, category, completed: false });
  ...
  useEffect(() => {
    syncRoomsWithCleaningTasks(cleaningTasks);
  }, [cleaningTasks]);
  ...
  useEffect(() => {
    const handler = (e: StorageEvent) => {
      if (e.key === "cleaning_task_list") {
        try {
          const parsed = e.newValue ? JSON.parse(e.newValue) : defaultCleaningTasks;
          setCleaningTasks(parsed);
          syncRoomsWithCleaningTasks(parsed);
        } catch (err) {
          // ignore
        }
      }
    ...
    const localHandler = (ev: Event) => {
      try {
        ...
        const parsed = (ev as CustomEvent).detail || JSON.parse(localStorage.getItem("cleaning_task_list") || "{}");
        setCleaningTasks(parsed);
        syncRoomsWithCleaningTasks(parsed);
      } catch (err) {}
    };
    window.addEventListener("cleaning_tasks_updated", localHandler as EventListener);
  ...
  const scrollCategories = (delta: number) => {
    ...
    // schedule update
    setTimeout(updateCatNav, 250);
  };

  useEffect(() => {
    updateCatNav();
    const handleResize = () => updateCatNav();
    window.addEventListener('resize', handleResize);
    return () => window.removeEventListener('resize', handleResize);
  }, [cleaningTasks]);

  // Track which activity was last selected per room (for a distinct highlight)
  const [selectedActivityPerRoom, setSelectedActivityPerRoom] = useState<Record<string, string | null>>({});
  ...
  const [activeActivityView, setActiveActivityView] = useState<Record<string, string | null>>({}); // Room ID -> active activity view (category key)
  ...
  const toggleActivity = (activityId: ActivityId, roomId: string) => {
    // mark selected activity for visual highlight
    setSelectedActivityPerRoom((prev) => ({ ...prev, [roomId]: activityId }));
    const room = rooms.find((r) => r.id === roomId);
    if (!room) return;
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-6">
                ...
                {selectedRooms[currentActivityRoomIndex] && (() => {
                  const room = selectedRooms[currentActivityRoomIndex];
                    const completedCount = room.activities.filter((a) => a.completed).length;
                    const totalCount = room.activities.length;
                    const currentCategoryKey = activeActivityView[room.id];
                    const categoryActivities = currentCategoryKey
                      ? room.activities.filter((act) => act.category === currentCategoryKey)
                      : [];
                  ...
                  return (
                    <div className="flex flex-col gap-6">
                      ...
                      <div className="relative">
                        {/* Left/Right chevrons for horizontal scroll */}
                        {/** Left button */}
                        <button
                          type="button"
                          onClick={() => {
                          ...
                          className="absolute left-0 top-1/2 -translate-y-1/2 z-20 rounded-full bg-white border shadow-sm p-1 hidden lg:inline-flex"
                          aria-label="Scroll left"
                        >
                          <ChevronLeft className="h-4 w-4" />
                        </button>

                        <div ref={catScrollRef} className="flex gap-2 overflow-x-auto pr-6 scrollbar-visible" style={{scrollbarWidth: 'auto'}}>
                          {Object.keys(cleaningTasks).map((catKey) => {
                          // display label
                          const label = catKey.split("-").map(w => w.charAt(0).toUpperCase()+w.slice(1)).join(" ");
                          const isActive = activeActivityView[room.id] === catKey;
                          return (
                            <Button
                              key={catKey}
                              size="sm"
                              onClick={() => toggleActivityView(room.id, catKey)}
                              className={`min-w-[120px] flex-1 justify-center whitespace-nowrap cursor-pointer transition-all px-3 py-2 ${
                                isActive
                        ...
                        <button
                          ...
                          className="absolute right-0 top-1/2 -translate-y-1/2 z-20 rounded-full bg-white border shadow-sm p-1 hidden lg:inline-flex"
                      ...
                      {/* Activities List */}
                      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-lg space-y-3 max-h-[500px] overflow-y-auto pr-3 scrollbar-visible" style={{scrollbarWidth: 'auto'}}>
                        {currentCategoryKey ? (
                          categoryActivities.length > 0 ? (
                            <div className="space-y-3">
                              {categoryActivities.map((activity) => {
                                const Icon = activity.icon;
                                const categoryActivitiesLocal = categoryActivities;
                                const currentCategoryIndex = categoryActivitiesLocal.findIndex((act) => act.id === activity.id);
                                const previousCategoryActivities = categoryActivitiesLocal.slice(0, currentCategoryIndex);
                                const allPreviousCompleted = previousCategoryActivities.every((act) => act.completed);
                                const isLocked = !allPreviousCompleted && !activity.completed;
                                const isSelected = selectedActivityPerRoom[room.id] === activity.id;

                                // determine classes: completed > selected > default
                                const baseBtnClass = `w-full rounded-2xl border-2 p-3 text-left shadow-sm transition-all`;
                                const stateClass = activity.completed
                                  ? "border-emerald-400 bg-emerald-50 cursor-pointer"
                                  : isLocked
                                  ? "border-rose-400 bg-rose-50 cursor-not-allowed opacity-75"
                                  : isSelected
                                  ? "border-sky-300 bg-sky-100 cursor-pointer"
                                  : "border-slate-200 bg-white hover:border-blue-300 cursor-pointer";

                                const iconBgClass = activity.completed
                            ...
                            <div className="text-center py-8 text-slate-500">
                              <p className="text-sm">No activities found for this category</p>
                            </div>
                          )
                        ) : (
                          <div className="text-center py-8 text-slate-500">
